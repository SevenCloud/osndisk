...
static const char *version_str = "0.1  2013/05/23";
...
static const char *usage_message =
    "Usage: osndisk  [--list] [--force] [--rescan] [--info] [--help]\n"
	"where:\n"
    "    --list|-l            list all disks detail information\n"
    "    --force|-f           force rescan new disks and update changed disks\n"
    "    --rescan|-r          rescan new disk\n"
    "    --info|-i            list disk information filter by <h:c:t:l> device tuple \n"
 ...
 */
static int
string_get_size(uint64_t size, const enum string_size_units units,
		char *buf, int len)
{
...
static int sub_scandir_select(const struct dirent *s)
{
	if (s->d_type == DT_LNK)
		return 1;
...
static int sd_scandir_select(const struct dirent *s)
{
 ...
 */
static int block_scandir_select(const struct dirent *s)
{
	if (s->d_type != DT_LNK && s->d_type != DT_DIR)
		return 0;
...
sub_scan(char *dir_name, const char *sub_str, dirent_select_fn fn)
{
	...
	if (num && strstr(dir_name, sub_str) == 0) {
		...
		num =
		    scandir(dir_name, &namelist, sub_scandir_select, NULL);
...
static int enclosure_device_scandir_select(const struct dirent *s)
{
	if ((DT_LNK != s->d_type) &&
	    ((DT_DIR != s->d_type) || ('.' == s->d_name[0])))
		return 0;
 ...
 */
static int
enclosure_device_scan(const char *dir_name,
		      const struct lsscsi_opt_coll *op)
{
	...
	num =
	    scandir(dir_name, &namelist, enclosure_device_scandir_select,
		    NULL);
	if (num < 0) {
		if (op->verbose > 0) {
			snprintf(name, NAME_LEN_MAX, "scandir: %s",
				 dir_name);
			perror(name);
		}
		return -1;
...
static int sas_port_scandir_select(const struct dirent *s)
{
...
static int sas_port_scan(const char *dir_name, struct dirent ***port_list)
{
	struct dirent **namelist;
	int num;
...
sas_low_phy_scan(const char *dir_name, struct dirent ***phy_list)
{
	...
	num =
	    scandir(dir_name, &namelist, sas_low_phy_scandir_select, NULL);
	if (num < 0)
		return -1;
	if (!phy_list) {
		for (k = 0; k < num; ++k)
...
static int iscsi_target_scandir_select(const struct dirent *s)
{
	...
	if (0 == strncmp("session", s->d_name, 7)) {
		...
		snprintf(buff + off, sizeof(buff) - off,
			 "/%s/target%d:%d:%d", s->d_name,
			 iscsi_target_hct->h, iscsi_target_hct->c,
			 iscsi_target_hct->t);
...
iscsi_target_scan(const char *dir_name, const struct addr_hctl *hctl)
{
	...
	num =
	    scandir(dir_name, &namelist, iscsi_target_scandir_select,
		    NULL);
	if (num < 0)
		return -1;
	for (k = 0; k < num; ++k)
...
static int get_dev_node(const char *wd, char *node, enum dev_type type)
{
...
static int collect_disk_wwn_nodes(void)
{
	...
	while (1) {
		...
		if ((k =
		     readlink(device_path, symlink_path, PATH_MAX)) < 1)
			continue;
		symlink_path[k] = '\0';
		...
		if (cur_list->count >= DISK_WWN_NODE_LIST_ENTRIES) {
			...
			if (!cur_list)
				break;
			memset(cur_list, 0,
			       sizeof(struct disk_wwn_node_list));
			prev_list->next = cur_list;
		}
...
/* Given a path to a class device, find the most recent device node with
   matching major/minor. Returns 1 if match found, 0 otherwise. */
static int get_disk_wwn(const char *wd, char *wwn_str, int max_wwn_str_len)
{
 ...
 */
static char *get_usb_devname(const char *hname, const char *devname,
			     char *b, int b_len)
{
	char buff[NAME_LEN_MAX];
		       ...
		       const struct lsscsi_opt_coll *op)
{
	...
	if (parse_colon_list(devname, &hctl)) {
		strcpy(buff, path);
		len = strlen(buff);
		snprintf(buff + len, NAME_LEN_MAX - len,
			 "/device/target%d:%d:%d/%d:%d:%d:%d", hctl.h,
			 hctl.c, hctl.t, hctl.h, hctl.c, hctl.t, hctl.l);
...
static int transport_init(const char *devname,	/* const struct lsscsi_opt_coll * op, */
			  int b_len, char *b)
{
	char buff[NAME_LEN_MAX];
	...
	do {
		...
		*t = 0;
		...
		if (strlen(buff) + strlen(buff2) + strlen("host_id/guid") +
		    2 > NAME_LEN_MAX)
			break;
		strcat(buff, buff2);
		      ...
		      const struct lsscsi_opt_coll *op)
{
	...
	case TRANSPORT_FCOE:
		...
		if (get_value
		    (buff, "max_npiv_vports", value, NAME_LEN_MAX))
			printf("  max_npiv_vports=%s\n", value);
		if (get_value
		    (buff, "npiv_vports_inuse", value, NAME_LEN_MAX))
		...
		if (get_value(buff, "speed", value, NAME_LEN_MAX))
			printf("  speed=%s\n", value);
		if (get_value
		    (buff, "supported_speeds", value, NAME_LEN_MAX))
			printf("  supported_speeds=%s\n", value);
		if (get_value
		    (buff, "supported_classes", value, NAME_LEN_MAX))
			printf("  supported_classes=%s\n", value);
		if (get_value
		    (buff, "tgtid_bind_type", value, NAME_LEN_MAX))
	...
	case TRANSPORT_SAS:
		...
		if ((portnum = sas_port_scan(buff, &portlist)) < 1) {	/* no configured ports */
			...
			for (i = 0; i < phynum; i++) {
				...
				printf("  %s\n", phylist[i]->d_name);
				if (get_value
				    (buff, "sas_address", value,
				     NAME_LEN_MAX))
					printf("    sas_address=%s\n",
					       value);
				if (get_value
				    (buff, "phy_identifier", value,
				     NAME_LEN_MAX))
					printf("    phy_identifier=%s\n",
					       value);
				if (get_value
				    (buff, "minimum_linkrate", value,
				     NAME_LEN_MAX))
					printf("    minimum_linkrate=%s\n",
					       value);
				if (get_value
				    (buff, "minimum_linkrate_hw", value,
				     NAME_LEN_MAX))
					printf
					    ("    minimum_linkrate_hw=%s\n",
					     value);
				if (get_value
				    (buff, "maximum_linkrate", value,
				     NAME_LEN_MAX))
					printf("    maximum_linkrate=%s\n",
					       value);
				if (get_value
				    (buff, "maximum_linkrate_hw", value,
				     NAME_LEN_MAX))
					printf
					    ("    maximum_linkrate_hw=%s\n",
					     value);
				if (get_value
				    (buff, "negotiated_linkrate", value,
				     NAME_LEN_MAX))
					printf
					    ("    negotiated_linkrate=%s\n",
					     value);
			}
			return;
		}
		for (i = 0; i < portnum; i++) {	/* for each host port */
			...
			if ((phynum =
			     sas_low_phy_scan(buff, &phylist)) < 1) {
			...
			if (get_value
			    (buff, "num_phys", value, NAME_LEN_MAX)) {
				printf("  %s: num_phys=%s,",
			...
			strcat(buff, sas_low_phy);
			if (get_value
			    (buff, "device_type", value, NAME_LEN_MAX))
				printf("    device_type=%s\n", value);
			if (get_value
			    (buff, "initiator_port_protocols", value,
			     NAME_LEN_MAX))
				printf("    initiator_port_protocols=%s\n",
				       value);
			if (get_value
			    (buff, "invalid_dword_count", value,
			     NAME_LEN_MAX))
				printf("    invalid_dword_count=%s\n",
				       value);
			if (get_value
			    (buff, "loss_of_dword_sync_count", value,
			     NAME_LEN_MAX))
			...
			if (get_value
				...
				printf("    negotiated_linkrate=%s\n",
				       value);
			if (get_value
			    (buff, "phy_identifier", value, NAME_LEN_MAX))
				printf("    phy_identifier=%s\n", value);
			if (get_value
			    (buff, "phy_reset_problem_count", value,
			     NAME_LEN_MAX))
				printf("    phy_reset_problem_count=%s\n",
				       value);
			if (get_value
			    (buff, "running_disparity_error_count", value,
			     NAME_LEN_MAX))
				printf
				    ("    running_disparity_error_count=%s\n",
				     value);
			if (get_value
			    (buff, "sas_address", value, NAME_LEN_MAX))
				printf("    sas_address=%s\n", value);
	...
	case TRANSPORT_SAS_CLASS:
		...
		if (get_value(buff, "ha_name", value, NAME_LEN_MAX))
			printf("  ha_name=%s\n", value);
		if (get_value
		    (buff, "version_descriptor", value, NAME_LEN_MAX))
		       ...
		       const struct lsscsi_opt_coll *op)
{
	...
	strcpy(buff, path_name);
	len = strlen(buff);
	snprintf(buff + len, NAME_LEN_MAX - len, "/scsi_device:%s",
		 devname);
	if (!if_directory_chdir(buff, "device"))
		return;
	if (NULL == getcwd(wd, NAME_LEN_MAX))
	...
	case TRANSPORT_FCOE:
		...
		*cp = '\0';
		...
		if (get_value
			...
			printf("  supported_classes=%s\n", value);
		if (get_value
		    (buff, "fast_io_fail_tmo", value, NAME_LEN_MAX))
	...
	case TRANSPORT_SAS:
		...
		if (get_value(buff, "bay_identifier", value, NAME_LEN_MAX))
		...
		if (get_value(buff, "phy_identifier", value, NAME_LEN_MAX))
			printf("  phy_identifier=%s\n", value);
		if (get_value
		    (b2, "ready_led_meaning", value, NAME_LEN_MAX))
	...
	case TRANSPORT_SAS_CLASS:
		...
		if (get_value(buff, "pathways", value, NAME_LEN_MAX))
			printf("  pathways=%s\n", value);
		if (get_value
		    (buff, "ready_led_meaning", value, NAME_LEN_MAX))
			printf("  ready_led_meaning=%s\n", value);
	...
	case TRANSPORT_ISCSI:
		...
		if (get_value(buff, "tpgt", value, NAME_LEN_MAX))
			printf("  tpgt=%s\n", value);
		if (get_value
		    (buff, "data_pdu_in_order", value, NAME_LEN_MAX))
			printf("  data_pdu_in_order=%s\n", value);
		if (get_value
		    (buff, "data_seq_in_order", value, NAME_LEN_MAX))
			printf("  data_seq_in_order=%s\n", value);
		if (get_value(buff, "erl", value, NAME_LEN_MAX))
			printf("  erl=%s\n", value);
		if (get_value
		    (buff, "first_burst_len", value, NAME_LEN_MAX))
		...
		if (get_value
		    (buff, "max_outstanding_r2t", value, NAME_LEN_MAX))
...
static void
osn_longer_d_entry(const char *path_name, const char *devname,
	       const struct lsscsi_opt_coll *op)
{
	char value[NAME_LEN_MAX];
	...
	if (op->long_opt >= 5) {
		if (get_value(path_name, "device_blocked", value, NAME_LEN_MAX)){
			printf("  %-22s","device_blocked");
			printf("%s\n", value);
		}else if (op->verbose > 0)
			printf("  device_blocked?\n");
		if (get_value(path_name, "iocounterbits", value,NAME_LEN_MAX)){
			printf("  %-22s","iocounterbits");
			printf("%s\n", value);
		}else if (op->verbose > 0)
			printf("  iocounterbits?\n");
		if (get_value(path_name, "iodone_cnt", value, NAME_LEN_MAX)){
			printf("  %-22s","iodone_cnt");
			printf("%s\n", value);
		}else if (op->verbose > 0)
			printf("  iodone_cnt?\n");
		if (get_value(path_name, "ioerr_cnt", value, NAME_LEN_MAX)){
			printf("  %-22s","ioerr_cnt");
			printf("%s\n", value);
		}else if (op->verbose > 0)
			printf("  ioerr_cnt?\n");
		if (get_value(path_name, "iorequest_cnt", value,NAME_LEN_MAX)){
			printf("  %-22s","iorequest_cnt");
			printf("%s\n", value);
		}else if (op->verbose > 0)
			printf("  iorequest_cnt?\n");
		if (get_value(path_name, "queue_depth", value,NAME_LEN_MAX)){
			printf("  %-22s","queue_depth");
			printf("%s\n", value);
		}else if (op->verbose > 0)
			printf("  queue_depth?\n");
		if (get_value(path_name, "queue_type", value,NAME_LEN_MAX)){
			printf("  %-22s","queue_type");
			printf("%s\n", value);
		}else if (op->verbose > 0)
			printf("  queue_type?\n");
		if (get_value(path_name, "scsi_level", value,NAME_LEN_MAX)){
			printf("  %-22s","scsi_level");
			printf("%s\n", value);
		}else if (op->verbose > 0)
			printf("  scsi_level?\n");
	       ...
	       const struct lsscsi_opt_coll *op)
{
	...
	if (op->long_opt >= 3) {
		...
		if (get_value(path_name, "iocounterbits", value,
			...
			printf("  iocounterbits=?\n");
		if (get_value
		    (path_name, "iodone_cnt", value, NAME_LEN_MAX))
			printf("  iodone_cnt=%s\n", value);
		else if (op->verbose > 0)
			printf("  iodone_cnt=?\n");
		if (get_value(path_name, "ioerr_cnt", value, NAME_LEN_MAX))
	       ...
	       const struct lsscsi_opt_coll *op)
{
	...
	if(op->long_opt>=3) printf("  %-22s","HDAddress");
...
//	if(op->long_opt>=3) printf("  %-22s","DeviceF");
	...
	if(op->long_opt>=3) printf("  %-22s","DiskSize");
	if (op->size) {
	...
	if (op->wwn)
		...
		if(op->long_opt>=3) printf("  %-22s","Vender");
		if (get_value(buff, "vendor", value, NAME_LEN_MAX)) {
			if(op->long_opt>=3) printf("%s\n", value);
			else printf("  %-8s ", value);
		} else{
		...
		if(op->long_opt>=3) printf("  %-22s","Model");
		if (get_value(buff, "model", value, NAME_LEN_MAX)) {
			if(op->long_opt>=3) printf("%s\n", value);
			else printf("%-16s  ", value);
		} else{
			if(op->long_opt>=3) printf("%s\n", "model?");
			printf("model?           ");
		}
		...
		if(op->long_opt>=3) printf("  %-22s","Rev");
		if (get_value(buff, "rev", value, NAME_LEN_MAX)) {
			if(op->long_opt>=3) printf("%s", value);
			else printf("%-4s", value);
		} else{
			if(op->long_opt>=3) printf("%s\n", "rev?");
			else printf("rev?  ");
		}
	...
	if (op->protection) {
		...
		if (sd_scan(sddir) &&
		    ...
		    get_value(".", "protection_type", value,
			      NAME_LEN_MAX)) {
...
static void list_sdevices(const struct lsscsi_opt_coll *op)
{
	char buff[NAME_LEN_MAX];
	char name[NAME_LEN_MAX];
	struct dirent **namelist;
...
longer_h_entry(const char *path_name, const struct lsscsi_opt_coll *op)
{
	...
	if (op->long_opt >= 3) {
		if (get_value(path_name, "can_queue", value, NAME_LEN_MAX))
			...
			printf("  can_queue=?\n");
		if (get_value
		    (path_name, "cmd_per_lun", value, NAME_LEN_MAX))
		...
		if (get_value(path_name, "host_busy", value, NAME_LEN_MAX))
			...
			printf("  host_busy=?\n");
		if (get_value
		    (path_name, "sg_tablesize", value, NAME_LEN_MAX))
		...
		if (2 == op->long_opt) {
			...
			if (get_value
			    (path_name, "state", value, NAME_LEN_MAX))
...
/* List host (initiator) attributes when --long given (one or more times). */
static void
osn_longer_h_entry(const char *path_name, const struct lsscsi_opt_coll *op)
{
	char value[NAME_LEN_MAX];
...
static void list_hosts(const struct lsscsi_opt_coll *op)
{
	char buff[NAME_LEN_MAX];
	char name[NAME_LEN_MAX];
	struct dirent **namelist;
...
static int one_filter_arg(const char *arg, struct addr_hctl *filtp)
{
	...
	for (k = 0; *cp; cp = cpe + 1, ++k) {
		...
		if (n > ((int) sizeof(buff) - 1)) {
			fprintf(stderr,
				"intermediate sting in %s too long "
				"(n=%d)\n", arg, n);
			return 1;
		}
		if ((n > 0) && ('-' != *cp) && ('*' != *cp)
		...
		default:
			fprintf(stderr,
				"expect three colons at most in %s\n",
				arg);
			return 1;
		}
	}
...
void osn_rescan_device(struct lsscsi_opt_coll *opts)
{
	...
	if (filter.h != -1) {
		snprintf(cmd + strlen(cmd),
			 OSN_COMMAND_LENGTH - strlen(cmd), " --hosts=%d",
			 filter.h);
		if (filter.c != -1) {
			snprintf(cmd + strlen(cmd),
				 OSN_COMMAND_LENGTH - strlen(cmd),
				 " --channels=%d", filter.c);
			if (filter.t != -1) {
				snprintf(cmd + strlen(cmd),
					 OSN_COMMAND_LENGTH - strlen(cmd),
	...
	if (!opts->verbose)
		snprintf(cmd + strlen(cmd),
			 OSN_COMMAND_LENGTH - strlen(cmd), " > /dev/null");
...
/*	if (opts.long_opt >= 3) {
		if (optind < argc) {
			...
			if (decode_filter_arg(a1p, a2p, a3p, a4p, &filter))
				return 1;
			if ((filter.h != -1) || (filter.c != -1) || (filter.t != -1)
					|| (filter.l != -1))
				filter_active = 1;
		}
		else{
			printf("No HDAddress specified, please supply <h:c:t:l> device tuple !\n");
			return 0;
		}
		if ((opts.transport > 0)
